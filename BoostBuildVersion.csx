...
// 用途:
//  1. 自動將主專案 Tekkon/Tekkon.csproj 的 <Version> 之 semver patch 自動 +1。
//  2. 或若第一個非旗標參數是合法 semver (X.Y.Z) 則直接使用該版本號。
//  3. 同步更新以下檔案中的版本字串：
//       - Tekkon/Tekkon.csproj: ReleaseVersion / AssemblyVersion / FileVersion / Version
//       - Tekkon.Tests/Tekkon.Tests.csproj: ReleaseVersion
//       - Tekkon.sln: MonoDevelopProperties 區段中的 version = X.Y.Z
...
string mainProj = Path.Combine(root, "Tekkon", "Tekkon.csproj");
...
if (!verMatch.Success) {
    Console.Error.WriteLine("無法在 Tekkon.csproj 找到 <Version> 標籤。");
    Environment.Exit(1);
}
...
string[] files = {
    Path.Combine(root, "Tekkon", "Tekkon.csproj"),
    Path.Combine(root, "Tekkon.Tests", "Tekkon.Tests.csproj"),
    Path.Combine(root, "Tekkon.sln")
};
    ...
    try {
        ...
        if (checkProcess?.ExitCode == 0) {
            ...
            var addPsi = new System.Diagnostics.ProcessStartInfo {
                FileName = "git",
                Arguments = "add Tekkon/Tekkon.csproj Tekkon.Tests/Tekkon.Tests.csproj Tekkon.sln",
                UseShellExecute = false
            };
